...
| `upgrade skills` | every skill or script line of the workspace's `TODO.md` read as its critic, a project's finding skipped, then taken to its rule or script, one commit each with its estimate, the line struck as it lands; a line that fails the read is asked, never struck; nothing pushed |
