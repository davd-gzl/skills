---
title: "Ten measured lessons from an AI code reviewer"
...
Changed, next: the finder runs the checks of its own Warning-band candidates
and writes the artifact; a judge, six candidates per agent, reruns the
artifact, reads the code and answers correct, incorrect or unproven, keeping
the base comparison for causal claims. Every production reviewer read for this
keeps an independent check after the finder and none repeats the finder's
work; Cloudflare's finder produces the executable proof and a validator that
cannot file findings tries to break it. Projected at measured constants: a
review from about $69 to about $53. Reusable: re-measure every stage against
the finders you have now, not the ones the stage was built for.
...
mechanical half of that list moves to a tool run before it. Reusable: the
author of a text does not see its errors; a second context does, cheaply.
...
| Word | Shape | PR 6187 |
| --- | --- | --- |
| `cheap review` | one finder per bundle on the Warning-finding angles, one verifier on the top six by band, the rest handed over with their checks named, one Nit batch, the writer | 8 agents, ~300k output, about $20 |
| `quick review` | the same finders, no reflector, short parallel verifier batches, the writer | 10 agents, ~290k, about $20, 40 minutes |
| `review` | one finder per bundle per angle, the reflector, verifier batches, Nit batches, the writer, the text pass | 32 agents, ~1.04M, ~60M cache, about $69 |
| `deep review` | `review` with the hot bundles twice, one verifier per candidate, Nits by six | 65 agents, ~1.9M, about $133 |

Each word has an output ceiling; past it no verifier is dispatched and the
writer still runs. `review` costs more than the sixty-dollar round because it
runs thirteen finders at xhigh where the old round ran six, chosen for recall;
the judge design brings it under.
