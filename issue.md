...
On `post`, run `./scripts/post-fix.sh` per *Presenting the change* in `skills/change.md`. To post standalone: `gh issue create -R <repo> --title "<title>" --body-file <path>`, body only, then write the URL into `Target:` yourself.
