...
- Stateless, like every inline comment: never name a round, never frame current code as a fix relative to a prior draft. State the code's current property, not its history. **A commit is history too**: no sha, no message, no ordering, no count of what landed when. Where the branch's own sequencing is the finding, the finding is what the code does now and what a reader has to split to fix it.
