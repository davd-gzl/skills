...
- A carried finding whose thread from an earlier round is still open goes as a reply in that thread, never as a new inline section: `replies.md` beside the draft, one section per thread, posted on `post` with `gh api -X POST repos/<repo>/pulls/<n>/comments/<id>/replies -F body=@<file>`. List the open threads before showing the draft, `gh api --paginate repos/<repo>/pulls/<n>/comments --jq '.[]|select(.user.login=="<login>" and .in_reply_to_id==null)|"\(.id) \(.path):\(.line)"'`, and move every section anchored on one.
