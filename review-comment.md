...
Auto-SKIP duplicates: when another reviewer already raised a finding, prefix its header with `SKIP` while drafting, attribute the reviewer in `claims.md`, and make `Already raised: <comment-url>` the section's first body line. **A duplicate the posting account raised itself gets no reaction**, since a thumbs-up on one's own comment is the reviewer agreeing with themselves; the `SKIP` and the `Already raised:` line stay, and the round posts what that account has not already said. When a section bundles an already-raised finding with a novel one, split it so the novel part posts. Where the raised finding is one case of a broader one being posted, name that case in the broader sentence and link it to the original instead of splitting.
...
- **`post as an AI` sends the draft with `> AI review, <model>, <preset> review, [skills](https://github.com/davd-gzl/skills) · [overview](<url>) · Status: <verdict>` as the Body's first line and `COMMENT` as the event**, whatever the verdict. `--as-ai` builds that string: it reads the visibility of the repository holding `overview.md`, carries the link at that repository's default branch when it is public and omits it when it is private. The model and preset come from the draft's `Model:` line, and the script refuses a line recording no preset. **Nothing is composed beside the marker**, and words the user typed for it go on its line behind a second `·`, verbatim. The word covers the one post it was said for, and it is the only phrase that puts an AI marker in a posted string.
