...
Model: <model>, effort <tier> (comment) <— comment mode only, per skills/review-modes.md>
...
- On a security fix, an advisory or an embargoed repository, post only what changes behaviour, the fix's coverage or the verdict. A Nit on a test's wording, a comment, a count or a name ships `SKIP` and stays in the review file: noise on that thread costs the reviewers reading it.
...
Then the QA agents the score in `skills/review.md` calls for, once each: the claim gate dispatched at step 7 of its workflow, the text pass over the finished draft; a target on the short path gets neither, and `claims.md` holds the parent's own rows. An edit after that gets the parent's own pass over the edited section, its cited lines re-read, and a run only where the edit adds or changes a claim: each agent pass reads the draft, the review file, the rules and every cited line, so a pass per edit costs the round again.

- **Apply the claim gate's findings before any rewording**: the agent in *Deep mode* step 5 of `skills/review-modes.md`, over the review file and the round's `tests/`, re-running every repro and every line number in them. The draft copies the file's finding text, so its claims are the file's rows. Its repro blocks are the parent's run, per *Repro rules* in `skills/review.md`. Its table is committed beside the draft as `claims.md`, and a sentence written after it was dispatched gets its own row, run, before the push. It kills whole findings, and a wording pass run before it polishes sentences that leave the draft.
...
- One gate and one text pass per target. The gate builds and runs the project's suites while the text pass only reads, so one agent covering several targets serialises the expensive half.
