...
- **`post as an AI` sends the draft with `[AI review]` as the Body's first line and `COMMENT` as the event**, whatever the verdict. It is the user making the disclosure themselves, so it is the only phrase that puts an AI marker in a posted string, and it covers the one post it was said for.
- A post made as an AI writes a `Status:` line into the draft, under the title, in the same turn. It names the phrase that authorised it, the event that went out and that the Body carries the marker, so a later session reads what was disclosed from the file rather than reopening the review.
