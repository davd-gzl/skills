...
- **The Body states, it never instructs.** `Read the caption path`, `Check every path that hands out a pass`: each reads as homework set for the author, who wrote the branch and knows the path better than the reviewer does. Say what the review looked at, or say the finding. The imperative also hides an empty line, since an instruction needs no evidence and asserts nothing that can be wrong.
