...
- **`post as an AI` sends the draft with `> AI review, <model> <tier> (not manually verified) · Status: <verdict>` as the Body's first line and `COMMENT` as the event**, whatever the verdict: `--as-ai` on the script. A blockquote, no brackets, the verdict on the same line behind a `·`, because a `COMMENT` event reaches nobody carrying it. The model and reasoning tier come from the review file's `Model:` line, and a tier it does not record is left out rather than guessed. The parenthetical is fixed text and never softened, dropped or reworded: it says the user did not check the output by hand, which stays true of a finding proved by execution. It is the user making the disclosure themselves, so it is the only phrase that puts an AI marker in a posted string, and it covers the one post it was said for.
