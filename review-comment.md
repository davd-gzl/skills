...
Then two QA agents, once each: the claim gate dispatched at step 7 of the workflow in `skills/review.md`, the text pass over the finished draft. An edit after that gets the parent's own pass over the edited section, its cited lines re-read, and a run only where the edit adds or changes a claim: each agent pass reads the draft, the review file, the rules and every cited line, so a pass per edit costs the round again.
