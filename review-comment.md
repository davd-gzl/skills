...
- **A mistake in the pull request's own description never reaches the draft.** The author rewrites their summary from the findings, and a reader of the code sees nothing that correction changes. Where the description is the only source of a claim, anchor the claim on the code carrying it. What the description got wrong stays in the review file.
