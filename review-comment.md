...
- **`post as an AI` sends the draft with `[AI review, <model> <tier>]` as the Body's first line and `COMMENT` as the event**, whatever the verdict. The model and reasoning tier come from the review file's `Model:` line, and a tier it does not record is left out rather than guessed. The marker names what wrote the review and stops: `AI review` already tells the author no human wrote it, so a clause disclaiming human scrutiny only hedges findings that were verified by execution. It is the user making the disclosure themselves, so it is the only phrase that puts an AI marker in a posted string, and it covers the one post it was said for.
