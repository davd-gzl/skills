...
- A finding about a code comment's own wording stays in the review file and never enters `comment_<model>.md`, whatever band it lands in. It changes no behaviour, so it never earns an inline slot. Keep the measurement that shows the comment wrong, and say it was not posted.
