...
- **The score names the severity, and every word restating it is derived rather than asserted.** CVSS 3.1 bands are None 0.0, Low 0.1-3.9, Medium 4.0-6.9, High 7.0-8.9, Critical 9.0-10.0, so a 6.8 reads Medium in the title, the `Severity` row, the index heading and every summary that counts findings by band. The heading an index files a finding under is triage, `Criticals` or `C1`, and names no severity at all. Where a word and a number disagree inside one artifact, the reader has already seen both.
