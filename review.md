...
- **The tests and refactor angles run under `deep` alone, and claims only where the diff carries a doc.** They return what the author acts on later, a Suggestion, a Nit, a missing test, and a round on the defect angles still reaches the defects: measured on one 3,000-line target, the three produced 88 of 168 verdicts and 5 of the 15 confirmed Warnings, every one of those five on a code file. A doc asserts what the code does at runtime and a code comment's wording ships `SKIP` either way, so `finder.claims_docs_only` earns the claims finder on a bundle carrying a `.md`, a `docs/` or an `adr/` path and on no other.
...
- **An angle the user names in the launch turn runs, whatever the word.** The parent adds it to `args.stages.finder.angles` and clears `claims_docs_only` where claims is the one named, so `review <target> +claims` is one word away from the deep angle without the deep round. A preset moves knobs, the Warning cap never among them. `quick` runs one finder per bundle on the Warning-finding angles, judges in short parallel batches and no reflector, and lowers the text pass's word floor rather than dropping the stage, so a candidate past a budget arrives as a question with its check named, the user runs it and flips the section to posted or `SKIP`. Every word names an output ceiling in the config, `budget_total`, that binds only when the word carries `+<n>`: the suffix gives the harness the counter the runner reads, the finders stopping at their share and the judges at the floor. Without it the ceiling is a projection, every stage launches, and the runner says so at launch.
