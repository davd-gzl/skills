...
- **Score the vector from what the attack requires, and never move a metric to reach a band.** The base score carries the mechanics only, so a severity word may sit above it for what the score cannot see, a realm being essential or close to launch: where it does, the `Severity` row says so in the same breath, since a bare High beside a 6.8 reads as an error. The heading an index files a finding under, `Criticals` or `C1`, is filing and names no severity at all. Every later restatement derives from the one stated band, never from that heading.
