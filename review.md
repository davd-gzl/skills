...
- `claims.md`, the record: one row per candidate the verifiers ran: state, band, `file:line`, the check, the observed output, the artifact under `tests/`. A refuted candidate keeps its row with the proving line, so a later round reads what was cleared and why. Then the completeness answers.
