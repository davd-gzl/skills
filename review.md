...
- **Report what the user loses, never the artifact that causes it.** A conflict, a deleted file, a moved import and a missing guard are mechanical facts, and a reader who is told one still has to work out what breaks. Finish that work before writing: name the action that stops working, for whom, and where. The test is a maintainer reading the line cold, deciding in one pass whether to care. "This branch edits a file main deleted" fails it; "after the rebase Escape no longer closes the chat" passes it, and it is the same finding.
- **When the base has moved, build the merged state and run it.** A claim about what a rebase costs is a claim about code nobody has produced yet, so produce it: apply the base's version of the disputed hunk into the running branch, exercise the path, then revert. That converts "the fix lands nowhere" into a measured loss, and it occasionally shows the loss is wider than the diff suggests, because the surrounding code moved too.
- **A dispatched event is not a run, for anything a user drives with a mouse or a keyboard.** `dispatchEvent(new KeyboardEvent(...))` skips the focus moves, the default actions and the library handlers a real press goes through, so it can close a panel that a real Escape leaves open. Drive the real browser with a real input, every time; a synthetic event is only a probe for which listener fired.
...
- Every `file:line` reference is a link to the reviewed repo's blob on the branch under review: `` [`file:line`](https://github.com/<repo>/blob/<head-branch>/<path>#L<line>) ``, ranges `#L<a>-L<b>`, `<head-branch>` from `gh pr view <n> --json headRefName`. Applies to every reference, including files and tests cited by name. Never a bare backticked `file:line`.
- Link the branch, not the sha. A branch link stays correct as the author pushes, where a sha link freezes the review to a commit nobody is looking at any more and has to be rewritten every round. Re-verify the anchors on each round, since lines move under a branch link. Pin a sha in exactly two cases: the branch is gone (merged and deleted), or the claim is about code that has since changed and the point is what it said then.
...
- A repro whose output is a failure says so immediately above that output, in one line. A fail-now, pass-once-fixed test prints `FAIL`, and a reader who meets `FAIL` cold concludes the repro is broken and stops reading. Framing it once at the top of the block does not survive a long heredoc in between: the line goes directly above the output it explains, naming what failed and why the failure is the finding.
...
- A fixture's header comment is three or four lines: what it asserts, the measurement, and that it fails at the reviewed head. The mechanism belongs in the finding, and repeating it above the test turns a fixture a maintainer would paste into a wall they have to read first. Keep a measured table, which is data; cut the prose around it.
- Write the repro in the harness the repo already uses for that surface, and find it by reading the test the diff itself adds. A defect on an endpoint the project covers with an integration fixture belongs in that fixture format, not in a unit test poking an internal function: it runs the real path end to end, drops straight into the fix as its regression test, and a maintainer recognises the shape without reading it. Copy the neighbouring fixture's structure, naming and assertions.
- Size the fixture against the threshold it asserts. A repro claiming a bound is exceeded must exceed it: check the input against the real constant, since a demonstration that lands under the limit asserts a refusal the fixed code will not perform, and the test then fails after the fix for the opposite reason.
...
- The verdict answers to the severities, and a surviving Warning rules out APPROVE. Approving over one says in the same breath that a maintainer could block and that the reviewer would not, and the contradiction is invisible because the two live in different sections. Reconcile it before shipping: either the finding is a Nit and the band was wrong, or the verdict is COMMENT. Reach for COMMENT when the change improves strictly on its base and the Warning is a defect it did not introduce; REQUEST CHANGES when the branch causes the Warning or ships it to users. Whichever way it lands, the verdict line names the Warning and says why it does or does not block, so a reader never has to infer it.
...
- **Never write a Body line whose only job is to fill the field.** "Two blockers, both inline." and "Both fixes are inline." say nothing a reader cannot see by scrolling, and a review that opens on filler reads as long before it has said anything. GitHub refuses an empty body on REQUEST_CHANGES and COMMENT, so when every finding looks anchored, one of them is not: the finding whose subject is the branch rather than a line, a stale base, a missing rebase, a conflict, belongs in the Body with its repro collapsed under it. Anchor what is about code, and let the Body carry what is about the branch.
...
- After a successful post, write the URLs back (`Posted: <review-url>` under the title, `[posted](<comment-url>)` on each anchor), commit and push. Do it in the same turn as the post, never later: the `Posted:` line is what makes a re-post rewrite the existing review instead of adding a second one, so a draft that is live but does not say so is armed to double-post silently. Before any post, check whether the target already carries a review from this author and reconcile the draft first.
