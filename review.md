...
- **A proposed edit is a behavioral claim, and it ships only after running both sides of it.** Naming a line to delete or change asserts two things, that the defect goes and that nothing else moves, and a source read reaches neither. Apply the edit in a worktree and run four cases: the defect case and the case the current code already handles, each on the branch and with the edit applied. Measured 2026-08-14: dropping one term from a condition was drafted as the fix after reading the surrounding lines and reasoning that it "costs nothing elsewhere", and the run showed it writing the browser's language over the stored one on a plain page load, reintroducing the exact bug the branch existed to fix. The term was the guard, not the defect.
- **Enumerate the case space before writing the finding, not after the question.** Where a finding turns on two sets that must agree, name every cell: in both, in the first only, in the second only, in neither. The neither cell is the one usually reachable today, and the one the finding understates. Measured 2026-08-14: a Critical read "a first sign-in from a French browser comes up in English"; the neither cell, a browser whose language the project carries nowhere, widened it to "whatever the browser's language" and moved the blast radius from one locale to every user the app does not translate for.
...
- **One screen, one run, and a row per state.** A clip that splits into panes to carry a comparison halves the resolution of both halves and doubles what the reader tracks, and the reader is watching a GIF in a comment box. Put one run on screen and carry the state in a ledger that gains a row per step, each row holding the inputs the claim rests on beside the value on screen, sampled live from the page. The last frame then tells the whole story to someone who never presses play, which is most readers.
- **A readout must never imply a transition that did not happen.** A cell reading "never set" above a later cell holding a value says the step between them set it, and the reader asks why the finding is a bug at all. Label the absence of the container, "no account yet", not the absence of the value, and name the column for what it holds rather than how it got there: "language on the account", never "saved on the account".
...
- **Post every finding the author should act on, and open with the one that changes what they do next.** What stays behind in the review file is what needs no action from them: a check CI already reports, a note about the branch's base, a finding already raised by someone else. Never drop a real finding to make the review shorter; shorten the finding instead.
...
- **Nothing about CI reaches the comment, whatever state the checks are in.** Red, green and waiting-for-approval are all readable on the pull request itself, and a first-time contributor whose workflows have not run reads that on the same page. A line explaining it was answered with "remove that". The one exception the Body keeps is the stale-base sentence named below.
...
- **A finding that needs a third explanation leaves the comment.** Two rewrites that both fail to land mean the finding rests on context the reader does not have, and the third attempt is not shorter, it is a longer one that fails too. Mark the section `SKIP` with a line saying why, keep it in the review file with its measurement, and say so in the reply. A latent risk whose trigger nobody has pulled is the usual case: the author cannot act on it and the reviewer cannot state it briefly.
...
- **A draft embedding media hosted elsewhere is stale until that host is pushed.** The file and the rendered draft disagree silently: the text is the new one and the picture is the previous one, under the same filename. After re-recording, push the media, then read the raw URL's byte count back and compare it to the file on disk before calling the draft current. A content delivery network serves the old bytes for minutes after the push, so the host's own API is the immediate answer and the raw URL is the one to poll.
...
7. Every `## <path>:<line>` header carries its `[gh]` link to the branch under review. The header is not posted, so this link exists for the person reading the draft, who otherwise has no way to open the code from the file.
8. Every embedded image resolves at its raw URL and the bytes match the file on disk. A draft whose text is current and whose picture is not renders as the previous round with no sign that anything is wrong.
