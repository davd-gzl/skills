...
- **Where a round keeps a finder stage, the finders run on the strongest model available, and a cheaper one is never the saving.** A finder that misses a defect costs the round its reason to exist, and the judge below it can only rule on what it was handed. Cut the finder count, the caps or the angles instead, each of which drops reading the round chose to skip rather than reading it did badly.
- Scale the round to the model that reads it. Where the harness cannot run the config's models, `./scripts/review-setup.sh --model <id>` pins every stage to the one that runs, presets included. Each stage's row of the `models` table in `scripts/workflows/review-pipeline.json` then scales two things: the bundles `round dispatch --scale` cuts, and the judges' batches. The setup prints the rows it took. A model with no row takes `default`, and a row other than the baseline's is an estimate until a re-judged round measures it.
...
- Name in `together` the files one finder must read at once where the bundles split them: a test and the code it asserts, a caller and the callee whose contract the diff changes, a doc and the code it describes. The runner merges their bundles while the merge stays under the ceiling the finder's model row sets, and logs a set past it.
- `quick` caps the class at simple; `deep` takes complex and `args.shape` names a class outright, the triage then naming only the files read together; a round with topics, the critical pass, is never triaged.
...
| Candidate | Who | Where |
| --- | --- | --- |
| a Critical, a Warning, a rewrite | one judge per `verifier.batch` run-shaped candidates of one bundle, six by default, three under `deep`, times the judge model's `batch_scale`; the artifact rerun from the file, the code read, the same check at the merge base where the claim is causal | one scratch worktree per judge, `git -C <head worktree> worktree add --detach <scratch>/judge-<n> <sha>`, the tree restored between candidates, removed at the end |
| a Missing test, a Suggestion, a Nit | one judge per `verifier.read_batch`, twelve, times the same scale, under `verifier.read_tools` calls; a read settles it and never a mutation, a Missing test being an absence a grep settles | the head worktree, untouched |
