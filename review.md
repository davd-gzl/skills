...
4. Print the plan first, `./scripts/review-plan.py`, and paste its table and projection in the reply that launches the run, so what is about to run and cost is on screen before it does. Then run the workflow, the workspace's `scripts/workflows/review-pipeline.js` by `scriptPath`, with those as `args`. Each stage is an agent reading only the rule sections its artifact needs:
