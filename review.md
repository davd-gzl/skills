...
- **A published vector links to the FIRST calculator and decodes itself.** The link is `https://www.first.org/cvss/calculator/3.1#CVSS:3.1/<vector>`, whose fragment no browser sends, so an embargoed vector reaches nobody; NVD's takes it as a query parameter, which does. Under it goes one row per metric, the code beside the metric's name, then its rating, then what that metric means for this finding, since a reader deciding how bad this is cannot get that from `AV:N`.
