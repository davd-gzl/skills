...
- **A published vector decodes itself, in the artifact, and stands alone without a link.** One collapsed row per metric: the code beside the metric's name, its rating, then what that metric means for this finding, written to this finding and never boilerplate. A reader deciding how bad this is cannot get that from `AV:N`, and an embargoed advisory that needs a third party to be readable has taken a dependency for nothing.
