...
- **Code that cannot run is the finding, never the reason to drop one.** A guard whose condition is impossible, a branch nothing reaches, a default the type forbids: post it, with what the reader loses by keeping it. Reasoning that it never fires is reasoning to the opposite conclusion.
