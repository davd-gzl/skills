...
- **A rule the diff moves to write time, or to a job that runs once, holds only for the writers it enumerated.** A deployment runs the previous version beside the new one, so a write that version accepts lands after the new one has swept and outlives it, and a bulk update, a data migration or a direct database session skips the layer carrying the rule the same way. Name the writers outside this process, and say whether a read re-checks the rule at runtime and whether the residue heals itself.
