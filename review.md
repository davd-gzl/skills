...
- **A band name is not a severity, and the scored row wins.** The heading an index files a finding under is triage: `Criticals`, `C1`, the directory it sits in. Before a summary counting findings by severity goes out, read each count against the artifact's own `Severity` row, since a body opening on three criticals over rows reading High, Medium and Medium contradicts itself in front of the reader.
