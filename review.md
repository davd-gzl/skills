...
- Name each target's class in the reply that launches the run, from the *Triage* table, beside the plan: the class, the one line that earns it, and where a Warning would sit. The triage stage names its own class inside the round and the round note carries it; `deep` skips that stage, so there the reply's class is the only one.
