...
| `quick review <target>`, `deep review <target>` | the same round on a preset: `quick` bounds the wall clock, a tool-call budget per verifier, small batches, no critic, no text pass, caps and effort unchanged; `deep` adds finders, the security angles run twice, the Nits get verified | `./scripts/review-plan.py --compare` |
