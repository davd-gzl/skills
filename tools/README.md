...
| `round` | `dispatch <repo> <base> <head> [--risk <risk.json>] [--catalog 1] [--scale <factor>] [--diff-dir <dir>] [--json <file>] [--out <file>]` | the changed files cut into bundles by category: code and tests by directory, small directories merged with a sibling, a bundle over the ceiling split by file, docs and config in one bundle, carrying the removed angle beside claims where it deletes a line, generated files skipped; each bundle lists the angles it has material for and the finders it earns, one per angle or one carrying every angle under the floor, so the finder count follows the diff; `--scale` multiplies the floor and the ceiling by the finder model's row, and the JSON carries both; `--diff-dir` writes each bundle's diff with its enclosing functions and a comment-blanked twin | 2 on a bad call |
| `round` | `assemble <round dir> [--repo <head worktree>] [--sha <sha>] [--risk <risk.json>] [--url <blob url base>] [--title <text>] [--shape <text>]` | `claims.md` and `findings.md` from the round's verdicts as data, every cell with the `--repo` worktree and any scratch worktree prefix cut: `candidates/*.json`, what each finder, the reflector and the critic returned, and `verdicts/*.json`, what each verifier returned; `claims.md` holds the Candidates table, one row per verdict joined to its candidate, exact line first and the nearest within five lines after, one row per candidate no verifier reached, the rows a finder settled, the hit rate per tier and an empty Completeness section; `findings.md` one block per finding in posting order, band then file then line, `SKIP` in front of a PLAUSIBLE Nit or Suggestion, so the writer composes the text and never retypes a row | 1 when a row's `file:line` is not at the head, 2 on a bad call |
| `round` | `check <round dir> [--overview <file>] [--private <file>] [--out <file>]` | the mechanical half of the text pass over the round's `comment_*.md` and the `overview.md` beside the directory: an em-dash outside a fence, a visible sentence ending in a question mark, a finding header without its `[gh]` link, a phrase that points at the page, a `Full review:` line; over those and `claims.md`, `findings.md`, `candidates/` and `verdicts/`, an absolute path outside the reviewed repo, and each name of the `--private` list as a whole word; one row per hit into `<round dir>/check.md`, so the text pass keeps only the judgement | 1 on any hit, 2 on a bad call |
...
- `src/round/`: `links.rs`, `prior.rs`, `risk.rs`, `dispatch.rs`, `assemble.rs` and `check.rs`, one subcommand each; `LineMap` in `prior.rs` maps a line from one sha to another through the hunks of `git diff -U0`; the risk weights sit in one table at the top of `risk.rs`; `mod.rs` holds the usage, the dispatch and the option parsing.
