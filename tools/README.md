...
- `src/round/`: `links.rs`, `prior.rs` and `risk.rs`, one subcommand each; `LineMap` in `prior.rs` maps a line from one sha to another through the hunks of `git diff -U0`; the risk weights sit in one table at the top of `risk.rs`; `mod.rs` holds the usage, the dispatch and the option parsing.
