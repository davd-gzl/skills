...
The output is the user testing the change themselves, in a browser, in under a minute of their attention: a URL, a login, the click path, and what to watch for. Never a command to run.

## Workflow

1. **Read `projects/<repo>/AGENTS.md` first**, its *Local development* and *Ports* sections, before touching the checkout. It carries the boot recipe measured on this machine: system packages, runtime versions, seed data, whitelisted ports. Skipping it re-buys every failure it lists.
...
9. **Hand over, and stop.** URL, login, the click path in numbered steps, and one line per changed surface naming what to look at. The user tries it themselves next; do not script a drive of the whole feature before they have looked.

## Boot recipes

Write the recipe into `projects/<repo>/AGENTS.md` the moment it is measured, each failure with its symptom, so the next run recognises it instead of diagnosing it again. A recipe from the project's own documentation is not measured until it has run here.

- **Backend from the checkout, frontend from the worktree**, when the diff is frontend-only. The backend is unchanged by the PR, so it boots once and serves every later run.
...
**The video is the last step, never the first.** Recording costs a full scripted drive, and every wording change to the finding costs it again. Order: the user tries it by hand, the claim settles, the recording proves the settled claim.

1. **Hand over first.** The user's own run is the fastest verification available and costs nothing here.
2. **Script the measurement, not the movie.** Once they report what they saw, drive the same path headless and print the state after each step. The table of states turns their observation into a finding and reruns in seconds. Save the script under the review's `tests/`.
3. **Show the shot list before recording, and wait.** One numbered line per shot: the claim it proves, the clicks and keys in order, and what the screen must show for the claim to hold. The user cuts the shots they do not want; a recorded minute nobody asked for costs both sides. A shot whose expected outcome cannot be written down is not understood well enough to film.
4. **Record only once the finding text is frozen**, and only on the word `video`. Reuse the measurement script; captions that contradict a later correction are the cost of recording early.
5. **Re-run the shot list against the recording** before sending it. A caption the run contradicts is a false statement in a file the user will forward.

The overlay is the point: a video of a UI moving proves nothing about what was typed. Mark each click and name every key on screen, so the reader sees the input, not only the result. Export a GIF, which renders inline in a GitHub comment where an mp4 does not, and keep the mp4 beside it for anything longer than a few seconds. Files land in the review's `media/` and reach the user in the chat.

A private workspace cannot embed the GIF in a comment on someone else's repo, so say so rather than shipping a link that renders broken.
...
- **Nothing reaches the target repository.** No commit, branch, push, comment or review. This skill boots a PR; the review skill judges it.
