...
Run this over every drafted artifact as the last step of writing it, against the file and not from memory, per artifact and per revision. Having read this file earlier in the session does not discharge it, and each of the eight steps is a search over the draft. When a draft comes back bloated or wrong, run the step that was skipped before proposing a new rule. Where the artifact's own skill mandates a loop, `skills/pr-body.md` for one, run that loop first and this pass over its result. Report the outcome in the reply: what it changed, or that a full pass changed nothing.
