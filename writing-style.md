...
- **Two counts decide whether a sentence needs rewriting, and neither is a judgement.** Count the words between the sentence's first word and its main verb: more than five is a rewrite, which catches a relative clause with its pronoun dropped, "an account holding a code the map has no entry for can never save a pick", and a nominal subject, "Telling that default apart from a real choice needs an extra value". Then count the subordinate clauses after that verb: more than one is a rewrite, which catches "Nothing can tell that default from a real choice until the account can hold a value meaning nobody has picked yet". Then count the verbs before the first comma: two means a finite clause is sitting inside the subject, which is the shape that garden-paths hardest. A short subject with a concrete verb passes all three and needs no defence, "Nothing retries a save the server refused", and so does a participle modifying the subject, "An account arriving without a language would also lose the timezone sync". Reading it again and finding it clear is not the test: the author parses their own sentence in one pass every time.
- **`but` never stands in for `except`.** "every browser but an English one" reads as a conjunction until the noun arrives. Write the rule without the exception where the exception does not change the outcome: every first sign-in comes up English, so "whatever the browser's language" is both shorter and truer than carving out the English case.
- **A pronoun reaches back one clause, never two.** `it`, `one`, `that` and a bare demonstrative are resolved by the reader from the nearest candidate, which is rarely the intended one: "the signature promises a fallback it no longer returns" and "an account that did arrive without one" both failed on this. Repeat the noun; the repetition costs less than the backtrack.
...
8. **Counts.** Per sentence: words before the main verb, more than five is a rewrite; subordinate clauses after it, more than one is a rewrite; verbs before the first comma, two is a rewrite. A participle modifying the subject is not a clause, "An account arriving without a language would also lose the timezone sync" passes; a finite clause inside the subject is, and does not. `./scripts/prose-check.sh` runs this step and the next over a comment draft.
9. **Promises.** For every section opening `Suggestion:`, the next word is the verb the author would type. A gerund is not one: "adding a language without adding it to the map" instructs nobody. No imperative means the prefix is wrong or the finding belongs in the review file.
...
- **A fact the reader cannot place is a puzzle, not data.** The rule above covers a number whose stake its own sentence already carries. It does not cover a fact stating a condition, where the reader has to work out what changes when the condition breaks: "the two lists hold the same four codes today" was answered with "what do you mean". Say what the condition holds off, in the same sentence, or cut the fact.
- **`Suggestion:` is a promise, and the sentence keeps it.** The prefix announces a change, so the first word after it is the verb the author would type: drop this term, export that constant, add this entry. `Nit:` carries no such promise and may state a defect plainly, "`!user.language` can never be true". A gerund is the near-miss to search for, since "adding a language without adding it to the map would leave every account unable to save" reads like an instruction and instructs nobody.
