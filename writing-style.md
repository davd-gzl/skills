...
- Cut filler: delete any clause the reader can infer from the rest of the sentence. A clause carrying a fact, a number or the reason to care is not filler however redundant it reads, and cutting it is deletion rather than shortening.
- Never sign-post from inside the document: no "see below", no "as mentioned above". Restructure instead.
- **Two rejections of one sentence end the guessing.** Offer three or four labelled variants, shortest first, and let the reader pick.
...
- **A connective states a relation that holds.** "while" and "though" promise a contrast, "so" and "because" promise cause: when the two facts merely sit beside each other, say they are related and stop.
...
- **A picture is introduced, never dropped in.** A blockquote before it, `> `, says what it shows and what produced it, naming the browser, the instance or the command. Capture it cropped to the thing in question, store the file under the artifact's `media/` directory, and embed it by its `raw.githubusercontent.com` URL. Read that URL back before shipping and compare its byte count against the file on disk: the raw URL serves the previous bytes for minutes after a replace. Where the screen cannot be reached from here, name the login or device it needed. A clip speeds up the stretch that argues nothing and keeps the elapsed time on screen.
...
- Anchor every link on words already in the prose, so no URL is ever visible text, and link a named doc subsection to its header line. Never write a sentence whose only job is to carry a link; fold it into a sentence with content of its own.
- In code comments, keep the symbols a contributor needs, and link the canonical source instead of restating it.
- Two lines is the budget for a code comment and one line for a docstring. Write the one fact the reader cannot get from the lines beside it, in the words of someone who has never read this codebase: a room id, not a UUID. Longer than that belongs in the pull request body.
- A docstring opens on what the caller gets back and never restates the line under it.
...
1. **Mechanical bans**, the em-dash, the parenthesis, and a hard-wrapped body or comment: `./scripts/prose-check.py` and `./scripts/rules lint` name every hit.
...
6. **Bare adjectives.** Search for "sound", "correct", "safe", "fine", "nothing broken", and for every adjective of degree, "large", "slow", "many", "old". Replace each with the number, or with the check that was run and what it showed, or delete it. A word ranking a threshold against the people it applies to, "ordinary", "small", "most", "few", goes the same way unless the distribution behind it was read: state the number and who can change it, and leave its size to the reader.
...
**Count the words the author reads, not the words in the file.** Link text counts; the URL inside it does not, nor do the `## path:line` headers, the fenced blocks, or anything collapsed behind `<details>`. The target is one finding written tightly, read off the reviewer's own recent posted comments rather than a figure here, and it is a density and never a budget for the review as a whole: a review carrying six findings costs six times it and is not over-long. Cut padding to reach it, never a finding. Every finding the author should see is anchored and posted; the total is whatever that comes to.
...
- No process words: no `Verified`, `measured`, `I ran`, `reproduced`, `on <sha>`. Evidence lives in the collapsed repro or `claims.md`, never in the visible line. A picture is the exception, and it keeps the sentence naming what it shows and what produced it.
- **A check that held is a `<details>`, never a line.** Visible text is for the result that went the other way: the guard that does not hold, the suite that reddens, the claim that failed. Everything else that was run, mutations included, goes in one collapsed block beside the repro, so a reader who wants the working opens it and nobody else pays for it.
...
- **A band naming an action carries it.** `Suggestion:` with nothing suggested is a defect report wearing the wrong label, and the author reads the band first: either the proposal goes into the sentence, or the band comes off and the finding takes the one its severity earns.
- A number carries its repro directly under it, collapsed, in the same comment. Never leave the repro in a private `claims.md` while the number goes public.
